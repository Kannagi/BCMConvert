...
int main(int argc, char** argv)
{
	...
	for(i = 1; i < argc;i++)
    {
        if(argv[i][0] == '-')
        {
            ...
            if(strcmp(argv[i],"-int")     == 0) option[7] = 1;
            if(strcmp(argv[i],"-fixedpoint") == 0) option[8] = 1;

            if(strcmp(argv[i],"-PS1") == 0) option[9] = 1;
            if(strcmp(argv[i],"-PS2") == 0) option[10] = 1;

            if(strcmp(argv[i],"-PS1vt") == 0) option[11] = 1;
    ...
    if(inname[0] == 0)
    {
		printf("Option : -nov -novt -novn - noindex -noanim -trisp -genN -int -fixedpoint -PS1 -PS2\n");
